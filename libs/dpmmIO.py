...
def process_sim_folder(args, suffix=''):
    ...
    if os.path.exists(raw_data_file):
        ...
        #Bhavya Changes here // Retreiving the time point information by calling the updated load_data function.
        args.true_data, args.time_points = load_data(raw_data_file, get_names=False)
...
def _get_out_dir(args, prefix=''):
    if args.output:
        ...
        res_dir = f'BnpC_ext_{args.time[0]:%Y%m%d_%H:%M:%S}{prefix}'
...
def show_model_parameters(data, args, fixed_errors_flag):
    ...
    #Bhavya Changes here // Print the time points
    if args.time_points is not None:
        print(f'\tTime points: {np.unique(args.time_points)}')
