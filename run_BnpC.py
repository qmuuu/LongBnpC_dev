...
def parse_args():
    ...
    # 20240625 Liting: Add number of timepoint
    model.add_argument('-times', '--numTimes', type=int, default = 1,
        help = 'Number of time points in this data.')
    model.add_argument(
        '-FN', '--falseNegative', type=list, nargs = '+', default=[-1],
        help='Fixed error rate for false negatives. A list with size numTimes.'
    )
    model.add_argument(
        '-FP', '--falsePositive', type=list, nargs = '+', default=[-1],
        help='Fixed error rate for false positives. A list with size numTimes.'
    )
    model.add_argument(
        '-Miss', '--missingRate', type=list, nargs = '+', default=[-1],
        help='Fixed error rate for missing rate. A list with size numTimes.'
    )
    model.add_argument(
    ...
    model.add_argument(
        '-Miss_m', '--missingRate_mean', type=check_ratio, default=0.2,
        help='Prior mean of the meaning rate. Default = 0.2.'
    )
    model.add_argument(
        '-Miss_sd', '--missingRate_std', type=check_ratio, default=0.1,
        help='Prior standard dev. of the missing rate. Default = 0.1.'
    )
    model.add_argument(
    ...
    if args.falseNegative != [-1] and len(args.falseNegative) != args.numTimes:
        parser.error(f'The -FN argument is set and requires exactly {args.numTimes} values.')
    if args.falsePositive != [-1] and len(args.falsePositive) != args.numTimes:
        parser.error(f'The -FP argument is set and requires exactly {args.numTimes} values.')   
    if args.missingRate != [-1] and len(args.missRate) != args.numTimes:
        parser.error(f'The -Miss argument is set and requires exactly {args.numTimes} values.')   
    return args
